...
resource "aws_instance" "spinnaker" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "/tmp/terraform/create_application.sh ${var.region} ${aws_vpc.main.id} ${var.username}-${var.prefix}-${var.base_iam_role_name} ${var.vpc_name}-${var.prefix}-${var.username} ${aws_security_group.example_app.id} ${aws_security_group.vpc_sg.id} ${aws_security_group.mgmt_sg.id}"
    ]
  }
